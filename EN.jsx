...
function main() {
    ...
    var tabs = [];
    var tabLabels =["Parameters", "Proportions", "Position"];
    for (var t = 0; t < 3; t++) {
    ...
    tab1.add("statictext", undefined, "Enter 12 digits of the barcode:");
    ...
    var quietZoneCheckbox = checksGroup.add("checkbox", undefined, "Add '>' marker on the right (Quiet Zone)");

    var whiteBgRow = checksGroup.add("group");
    whiteBgRow.orientation = "row";
    whiteBgRow.alignChildren = "center";

    var whiteBgCheckbox = whiteBgRow.add("checkbox", undefined, "Add white background");
    whiteBgCheckbox.value = false;

    whiteBgRow.add("statictext", undefined, "Padding:");
    ...
    whiteBgRow.add("statictext", undefined, "Right:");
    ...
    tab2.add("statictext", undefined, "Barcode size (" + unitLabel + "):");
    ...
    tab2.add("statictext", undefined, "Bar height (% of barcode height):");
    ...
    tab3.add("statictext", undefined, "Barcode position:");

    var positionGroup = tab3.add("group");
    positionGroup.orientation = "row";
    positionGroup.alignChildren = "center";

    var centerCheckbox      = positionGroup.add("checkbox", undefined, "Center");
    var bottomRightCheckbox = positionGroup.add("checkbox", undefined, "Bottom right");
    var customCheckbox      = positionGroup.add("checkbox", undefined, "Custom:");
    ...
    var originLabel = originBlock.add("statictext", undefined, "Origin point:");
    ...
    okButton.onClick = function () {
        ...
        if (!input.match(/^\d{12}$/)) {
            alert("Error: enter exactly 12 digits for the barcode!");
            switchTab(0);
            return;
        }
        ...
        if (isNaN(width) || width <= 0 || isNaN(height) || height <= 0) {
            alert("Error: specify positive numeric values for width and height!");
            switchTab(1);
            return;
        }
        ...
        if (isNaN(barHeightPct) || barHeightPct <= 0 || barHeightPct > 100) {
            alert("Error: bar height must be between 1 and 100%!");
            switchTab(1);
            return;
        }
        ...
        if (whiteBgCheckbox.value) {
            ...
            if (isNaN(padPct) || padPct < 0 || isNaN(padRPct) || padRPct < 0) {
                alert("Error: specify positive numeric values for the white background padding!");
                switchTab(0);
                return;
            }
        }
        ...
        if (doRot) {
            ...
            if (isNaN(rotAngle)) {
                alert("Error: specify a numeric value for the rotation angle!");
                switchTab(2);
                return;
            }
        }
        ...
        if (posMode === "custom") {
            ...
            if (isNaN(customX) || isNaN(customY)) {
                alert("Error: specify numeric values for X and Y!");
                switchTab(2);
                return;
            }
    ...
    if (addWhiteBg) {
        ...
        if (barcodeTargetHeightPt <= 0) {
            alert("Error: the specified height is too small for the white background padding!");
            return;
        }
    }
