...
function main() {
    ...
    var dialog = new Window("dialog", "Генератор бар-кода EAN-13");
    ...
    var tabs = [];
    var tabLabels =["Параметры", "Пропорции", "Позиция"];
    for (var t = 0; t < 3; t++) {
    ...
    tab1.add("statictext", undefined, "Введите 12 цифр бар-кода:");
    ...
    var quietZoneCheckbox = checksGroup.add("checkbox", undefined, "Добавить метку '>' справа (Quiet Zone)");

    var whiteBgRow = checksGroup.add("group");
    whiteBgRow.orientation = "row";
    whiteBgRow.alignChildren = "center";

    var whiteBgCheckbox = whiteBgRow.add("checkbox", undefined, "Добавить белый фон");
    whiteBgCheckbox.value = false;

    whiteBgRow.add("statictext", undefined, "Отступ:");
    var padInput = whiteBgRow.add("edittext", undefined, "5");
    padInput.characters = 3;
    padInput.enabled = false;
    whiteBgRow.add("statictext", undefined, "%");

    whiteBgRow.add("statictext", undefined, "Справа:");
    var padRInput = whiteBgRow.add("edittext", undefined, "10");
    padRInput.characters = 3;
    padRInput.enabled = false;
    whiteBgRow.add("statictext", undefined, "%");

    var previewPanel = tab1.add("panel", undefined, "Предпросмотр:");
    ...
    function drawSharedPreview(canvas) {
        ...
        var padPctStr = padInput.text.replace(",", ".");
        var padPct = parseFloat(padPctStr);
        if (isNaN(padPct) || padPct < 0) padPct = 5;

        var padRPctStr = padRInput.text.replace(",", ".");
        var padRPct = parseFloat(padRPctStr);
        if (isNaN(padRPct) || padRPct < 0) padRPct = 10;

        var pad = contentW * (padPct / 100);
        var padR = contentW * (padRPct / 100);
    ...
    whiteBgCheckbox.onClick = function () {
        padInput.enabled = whiteBgCheckbox.value;
        padRInput.enabled = whiteBgCheckbox.value;
        forceRedraw();
    };
    padInput.onChanging = forceRedraw;
    padRInput.onChanging = forceRedraw;
    ...
    tab2.add("statictext", undefined, "Размер бар-кода (" + unitLabel + "):");
    ...
    tab2.add("statictext", undefined, "Высота штрихов (% от высоты бар-кода):");

    var barHeightGroup = tab2.add("group");
    barHeightGroup.orientation = "row";
    barHeightGroup.alignChildren = "center";
    barHeightGroup.add("statictext", undefined, "Процент:");
    var barHeightInput = barHeightGroup.add("edittext", undefined, "76");
    barHeightInput.characters = 4;
    barHeightGroup.add("statictext", undefined, "(76% = стандарт, длинные +10%)");
    barHeightGroup.margins = [0, 0, 0, 6];
    ...
    var previewPanel2 = tab2.add("panel", undefined, "Предпросмотр:");
    ...
    tab3.add("statictext", undefined, "Позиция бар-кода:");

    var positionGroup = tab3.add("group");
    positionGroup.orientation = "row";
    positionGroup.alignChildren = "center";

    var centerCheckbox      = positionGroup.add("checkbox", undefined, "По центру");
    var bottomRightCheckbox = positionGroup.add("checkbox", undefined, "Справа снизу");
    var customCheckbox      = positionGroup.add("checkbox", undefined, "Произвольно:");
    ...
    var originLabel = originBlock.add("statictext", undefined, "Точка отсчёта:");
    ...
    var rotationCheckbox = rotationBlock.add("checkbox", undefined, "Поворот");
    ...
    rotationInputGroup.add("statictext", undefined, "градусов");
    ...
    var cancelButton = buttonGroup.add("button", undefined, "Отмена", {name: "cancel"});
    ...
    okButton.onClick = function () {
        ...
        if (!input.match(/^\d{12}$/)) {
            alert("Ошибка: введите ровно 12 цифр бар-кода!");
            switchTab(0);
            return;
        }
        ...
        if (isNaN(width) || width <= 0 || isNaN(height) || height <= 0) {
            alert("Ошибка: укажите положительные числовые значения ширины и высоты!");
            switchTab(1);
            return;
        }
        ...
        if (isNaN(barHeightPct) || barHeightPct <= 0 || barHeightPct > 100) {
            alert("Ошибка: высота штрихов должна быть от 1 до 100%!");
            switchTab(1);
            return;
        }

        var padPct = 5, padRPct = 10;
        if (whiteBgCheckbox.value) {
            padPct = parseFloat(padInput.text.replace(",", "."));
            padRPct = parseFloat(padRInput.text.replace(",", "."));
            if (isNaN(padPct) || padPct < 0 || isNaN(padRPct) || padRPct < 0) {
                alert("Ошибка: укажите положительные числовые значения отступов белого фона!");
                switchTab(0);
                return;
            }
        }
        ...
        if (doRot) {
            ...
            if (isNaN(rotAngle)) {
                alert("Ошибка: укажите числовое значение для угла поворота!");
                switchTab(2);
                return;
            }
        }
        ...
        if (posMode === "custom") {
            ...
            if (isNaN(customX) || isNaN(customY)) {
                alert("Ошибка: укажите числовые значения X и Y!");
                switchTab(2);
                return;
            }
    ...
    var bgPadPct  = result.padPct  / 100;
    var bgPadRPct = result.padRPct / 100;
    ...
    var barcodeTargetWidthPt  = targetWidthPt;
    var barcodeTargetHeightPt = targetHeightPt;

    if (addWhiteBg) {
        barcodeTargetWidthPt  = targetWidthPt / (1 + bgPadPct + bgPadRPct);
        barcodeTargetHeightPt = targetHeightPt - (barcodeTargetWidthPt * bgPadPct * 2);

        if (barcodeTargetHeightPt <= 0) {
            alert("Ошибка: заданная высота слишком мала для отступов белого фона!");
            return;
        }
    }

    var scaleX = (barcodeTargetWidthPt  / currentWidthPt)  * 100;
    var scaleY = (barcodeTargetHeightPt / currentHeightPt) * 100;
    ...
    if (addWhiteBg) {
        posPad  = bcW * bgPadPct;
        posPadR = bcW * bgPadRPct;
        boxW = bcW + posPad + posPadR;
        boxH = bcH + posPad * 2;
    }
    ...
    if (posMode === "center") {
        ...
        var boxLeft = abCX - boxW / 2;
        var boxTop  = abCY + boxH / 2;
        barcodeGroup.position = addWhiteBg
            ? [boxLeft + posPad, boxTop - posPad]
            : [boxLeft, boxTop];

    } else if (posMode === "bottomRight") {
        var marginPt = 2.83465;
        var boxRight  = abBounds[2] - marginPt;
        var boxBottom = abBounds[3] + marginPt;
        var boxLeft = boxRight - boxW;
        ...
        if (originH === "left") {
            ...
            boxLeft = abLeft + xPt - boxW;
        } else {
            boxLeft = abLeft + xPt - boxW / 2;
        }

        if (originV === "top") {
            boxTop = abTop - yPt;
        } else if (originV === "bottom") {
            boxTop = abTop - yPt + boxH;
        } else {
            boxTop = abTop - yPt + boxH / 2;
        }
    ...
    if (addWhiteBg) {
        ...
        var pad = bcW2 * bgPadPct;
        var padR = bcW2 * bgPadRPct;
